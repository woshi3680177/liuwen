...
NFC can prevent this kind of attack to a great extent, but MST is difficult to prevent. The middleman can completely interfere with the transaction when the mobile phone interacts with the PoS, and then after the MST information is obtained, it can be consumed on other PoS machines[46].
...
\begin{center}
\textbf{Table 1}~~Comparison.\\
 \setlength{\tabcolsep}{1mm}{\begin{tabular}{|p{2cm}|p{2cm}|p{2cm}|p{2cm}|p{2cm}|} %l(left)居左显示 r(right)居右显示 c居中显示
\hline 
&QR code&Audio&MST&NFC\\
\hline 
Information transmission&Coupling of magnetic filed&&&\\
\hline
